...
class SimpleBlock(nn.Module):
  ...
  def __init__(self, indim, outdim, half_res, leaky=False):
    ...
    if self.maml:
      ...
      self.BN1 = nn.BatchNorm2d(outdim, track_running_stats=TRACKBN)
      self.C2 = nn.Conv2d(outdim, outdim,kernel_size=3, padding=1,bias=False)
      self.BN2 = nn.BatchNorm2d(outdim, track_running_stats=TRACKBN)
    ...
    if indim!=outdim:
      if self.maml:
        ...
        self.BNshortcut = nn.BatchNorm2d(outdim, track_running_stats=TRACKBN)
...
class ConvNet_medium(nn.Module):
  def __init__(self, depth, flatten = True):
...
class ResNet(nn.Module):
  ...
  def __init__(self,block,list_of_num_layers, list_of_out_dims, flatten=True, leakyrelu=False):
    ...
    if self.maml:
      ...
      bn1 = nn.BatchNorm2d(64, track_running_stats=TRACKBN)
...
# --- Conv networks ---
def Conv4_small(flatten, leakyrelu):
    return ConvNet_small(4)
def Conv4_medium(flatten, leakyrelu):
    return ConvNet_medium(4)
def Conv4(flatten, leakyrelu):

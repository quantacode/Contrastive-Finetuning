...
class UFSL(ProtoNet):
  ...
  def train_target(self, epoch, tgt_loader, optimizer, Nmb, batches_pb):
    avg_loss=0
    print_freq = 10
  ...
  def train_joint(self, epoch, src_loader, tgt_loader, optimizer, loss_wt, Nmb, batches_pb):
    avg_loss_src = 0
    avg_loss_tgt = 0
    print_freq = 10
  ...
  def train_fsl_L1(self, epoch, src_loader, tgt_loader, optimizer, loss_wt):
    ...
    for i in range(len(src_loader)):
      ...
      loss_tgt,avg_dist  = self.sparsity_regularizer(x_tgt, self.projected_feature, y_tgt)
      ...
      if (i+1)%Nmb==0:
        ...
        if (i+1) % print_freq == 0:
          ...
          self.tf_writer.add_scalar(self.method + '/train/avg target dist', avg_dist.item(), epoch*len(src_loader) +
                                    i+1)
        avg_loss = 0
    return
  ...
  def validate_contrastive(self, epoch, test_loader):
    tau=0.1
    loss_projhead = 0.
    for i, (x, y) in enumerate(test_loader):
      n_way = x.size(0)
      ...
      loss_this_projhead = -li.sum(dim=0)

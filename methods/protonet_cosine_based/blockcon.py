...
class UFSL(ProtoNet_cosine):
  ...
  def _blockcon_loss(self, z, n_way, n_s):
    # normalize
    z = F.normalize(z, dim=1)
    bsz, featdim = z.size()
    mean_anchors = z.view(n_way, n_s, -1).mean(dim=1)
    mean_anchors = mean_anchors.unsqueeze(0).repeat(bsz,1,1)
    z = z.view(bsz, 1, featdim).repeat(1, n_way, 1)
    Sv = nn.CosineSimilarity(dim=2)(z, mean_anchors)
    coeff_mask = torch.FloatTensor(np.kron(np.eye(n_way), np.ones((n_s, 1)))).to(Sv.device)
    Sv = Sv + (1.0/(n_s-1))*coeff_mask*Sv - (1.0/(n_s-1))*coeff_mask
    Sv = torch.exp(Sv / self.tau)
    denominator = Sv.sum(dim=1)
    numerator = (Sv*coeff_mask).sum(dim=1)
    loss = -torch.log(numerator/denominator).sum(dim=0)
    return loss

  def blockcon_loss(self, x, mode='train'):
    n_way = x.size(0)
    n_s = x.size(1)
    x = x.cuda()
    ...
    if mode == 'train':
      return self._blockcon_loss(z, n_way, n_s)
    elif mode == 'val':
      lp = self._blockcon_loss(z, n_way, n_s)
      return lp
  ...
  def train_target(self, epoch, tgt_loader, optimizer):
    ...
    for i, (x_tgt, y_tgt) in enumerate(tgt_loader):
      ...
      loss = self.blockcon_loss(x_tgt, 'train')
      optimizer.zero_grad()
      loss.backward()
      optimizer.step()
      if i % print_freq == 0:
        ...
        self.tf_writer.add_scalar(self.method + '/blockcon_loss/total', loss.item(), bch_id)
    return

  def validate(self, epoch, test_loader):
    loss_projhead = 0.
    for i, (x, y) in enumerate(test_loader):
      loss_this_projhead  = self.blockcon_loss(x, 'val')

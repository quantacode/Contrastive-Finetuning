...
class UFSL(ProtoNet_cosine):
  ...
  def distribute_model1(self):
      self.projected_feature = nn.DataParallel(self.projected_feature)
      return self

  def set_forward_loss_target(self, x, feature_model, n_way, n_support, n_query):
  ...
  def mvcon_loss(self, x, feature_model):
    n_way = x.size(0)
    n_s = x.size(1)
    x = x.cuda()
    x = x.contiguous().view(n_way * n_s, *x.size()[2:])
    z = feature_model.forward(x)
  ...
  def train_target(self, epoch, tgt_loader, optimizer):
    ...
    for i, (x_tgt, _) in enumerate(tgt_loader):
      ...
      loss = self.mvcon_loss(x_tgt, self.projected_feature)
      optimizer.zero_grad()
      loss.backward()
      optimizer.step()
      if i % print_freq == 0:
        bch_id = epoch * len(tgt_loader) + i
        print('Batch {:d} | Loss {:f}'.format(bch_id, loss.item()))
  ...
  def test_unsupervised(self, test_loader):
    ...
    for i, (x, y) in enumerate(test_loader):
      ...
      if self.change_way:
        self.n_way = x.size(0)
      scores, loss = self.set_forward_loss_target(x, self.projected_feature, self.n_way, self.n_support_u, self.n_query_u)
  ...
  def test_supervised(self, test_loader, record = None):
    ...
    for i, (x,y) in enumerate(test_loader):
      ...
      if self.change_way:
        self.n_way  = x.size(0)
      scores, loss = self.set_forward_loss_target(x, self.projected_feature, self.n_way, self.n_support, self.n_query)

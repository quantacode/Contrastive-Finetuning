...
class Npcon_Model(SIMCLR):
  def __init__(self, model_func, tau, affinity_file, image_size, naug, n_way, tf_path=None,
               loadpath=None, loadpath_aux=None, is_distribute=False):
    ...
    self.naug = naug
    self.auxiliary_net = model_func(flatten=True, leakyrelu=False)
    for param in self.named_parameters():
      if 'auxiliary_net' in param[0]:
        param[1].requires_grad = False
    self.load_models(loadpath, loadpath_aux)
    # self.projection_nearpos = nn.Sequential(nn.Linear(self.feature.final_feat_dim, self.projected_feature_dim, bias=True))
    # #if DEBUG
    # self.projection_head = self.projection_nearpos
    #else DEBUG
    # self.projection_augpos = self.projection_head
    if is_distribute:
      self.distribute_model1()

  def load_models(self, loadpath, loadpath_aux):
    ...
    if loadpath_aux is not None:
      state = torch.load(loadpath_aux)['state']
      loadstate = {}
      for key in state.keys():
        newkey = key
        if '.module' in key:
        ...
        loadstate[newkey] = state[key]
      self.auxiliary_net.load_state_dict(loadstate, strict=False)
    return self
  ...
  def get_affinity(self, epoch):
    ...
    for i, (name, x, y) in enumerate(progress):
      ...
      # feat = self.auxiliary_net(x).detach()
    ...
    # uniform ensembling
    affinity_matrix = affinity_matrix.mean(dim=2)
  ...
  def train_npcon_contrastive(self, epoch, tgt_con_loader, tgt_loader, optimizer, nploss_wt):
    # shuffle the projection heads for the two losses, simclr and nearpos
    ...
    for i in progress:
      ...
      self.projection_head = self.projection_augpos
    ...
    # make sure contrastive projection head is default
    self.projection_head = self.projection_augpos
    return

  def train_npcon(self, epoch, tgt_loader, optimizer, mode='nearpos'):
     # train npcon based on source-model metric
    if mode=='nearpos':
      forward_func = self.kNearest_mvclr
    else:
      forward_func = self.simclr_loss
...
# ################ Class functions
  ...
  #   data = {'image_name': [], 'image_feat': torch.FloatTensor([]).cuda(),
  #           'image_feat_aux': torch.FloatTensor([]).cuda(), 'image_label': [], 'cluster_label': []}
  #   progress = tqdm(data_loader)
  #   progress.set_description('generating features')
  #   for i, (name, x, y) in enumerate(progress):
  #     data['image_name'].extend(name)
  ...
  #     data['image_label'].extend(y)
  #     x = x.cuda()
  #     feat = F.normalize(self.feature(x), dim=1).detach()
  #     data['image_feat'] = torch.cat([data['image_feat'], feat], dim=0)
  #     feat = F.normalize(self.auxiliary_net(x), dim=1).detach()
  #     data['image_feat_aux'] = torch.cat([data['image_feat_aux'], feat], dim=0)
  #
  #   affinity_matrix = self.group_cosine_dist(data['image_feat'])
  #   affinity_matrix_aux = self.group_cosine_dist(data['image_feat_aux'])
  #
  #   # # ensemble affinity
  #   # affinity_matrix = 0.5*(affinity_matrix/affinity_matrix.sum(dim=1).unsqueeze(1) + \
  #   #                   affinity_matrix_aux/affinity_matrix_aux.sum(dim=1).unsqueeze(1))
  #   # affinity_matrix = affinity_matrix.numpy()
  #
  #   # auxiliary only affinity
  #   affinity_matrix = affinity_matrix_aux.numpy()
  #
  #   # # current only affinity
  #   # affinity_matrix = affinity_matrix.numpy()
  #
  #   mode = np.max([int(np.ceil(self.naug / 2)), 2])
  #   if 'CUB' in self.affinity_file:
